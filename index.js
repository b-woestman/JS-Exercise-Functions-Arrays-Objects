 ...
 * }
*/
function makePersonObject(number , realname , emailaddress) {
  const user = {
    id: number,
    name: realname,
    email: emailaddress
...
makePersonObject(42, "Brandon", "bcwoestman@gmail.com");
...
function appleIndex(array, length) {
  for (i = 0 ; i < length ; i++){
    if (array[i] === "apple"){
      console.log(i);
      return i
...
*/
var fruit2 = [ 'orange', 'apple', 'banana', 'apples', 'apple', 'mango' ];
var length2 = fruit2.length;

function isItAnApple(array, length) {
  for (i = 0 ; i < length ; i++){
    if (array[i] === "apple"){
      array[i] = "true"
    } else {
...
function get3rdCar(inventory) {
  ...
  `The is a ${the3rd.car_make} ${the3rd.car_model}`
}
...
*/
const library = require('./data/inventory.js');

function getLastCarInfo(array) {
  const length = array.length;
  const last = array[length];
  console.log(`The is a ${last.car_make} ${last.car_model}`);
}
getLastCarInfo(library);
